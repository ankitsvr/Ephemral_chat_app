...
def add_friend():
    ...
    while new_friend.salutation.upper()!='MR' or new_friend.salutation.upper()!="MS":
        print "this is not poper salutation"
        raw_input("press enter to continue and enter proper salutation")
...
print "\n\t\t\t\t\t\t****************welcomw to spy chat application*****************\n\n"
...
if user_choice.upper() == "Y":
    ...
    update_char=spy.name[0]
    replace_char=update_char.upper()
    spy.name=spy.name.replace(update_char,replace_char)

    if len(spy.name)>0:

        spy.salutation = raw_input("Should I call you Mr. or Ms.?: ")

#convert saluation First letter in upper case
        update_char = spy.salutation[0]
        replace_char = update_char.upper()
        spy.salutation = spy.salutation.replace(update_char, replace_char)

        spy.age=raw_input("enter the age : ")
        spy.age=int(spy.age)
